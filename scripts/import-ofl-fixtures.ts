...
function getMinMaxValues(capability: OFLCapability): {
  min: number;
  max: number;
} {
  if (capability.dmxRange) {
...
async function importOFLFixtures() {
  ...
  const oflPath = path.join(parent, "open-fixture-library");
  ...
  for (const manufacturerKey of Object.keys(manufacturersData)) {
    ...
    for (const fixtureFile of fixtureFiles) {
      ...
      try {
        ...
        console.log(
          `  ✓ Imported: ${oflFixture.name} (${channelDefinitions.length} channels)`,
        );
        successfulImports++;
      } catch (error) {
