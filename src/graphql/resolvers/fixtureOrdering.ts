...
export const fixtureOrderingResolvers = {
  Mutation: {
    reorderProjectFixtures: async (
      ...
      {
        fixtureOrders,
      }: { fixtureOrders: Array<{ fixtureId: string; order: number }> },
      { prisma }: Context,
    ) => {
      try {
        ...
        throw new Error(
          `Failed to reorder project fixtures: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    },
    ...
    reorderSceneFixtures: async (
      ...
      try {
        ...
        throw new Error(
          `Failed to reorder scene fixtures: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    },
  },
