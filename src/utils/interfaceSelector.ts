...
export async function selectNetworkInterface(): Promise<string | null> {
  ...
  if (process.env.ARTNET_BROADCAST) {
    console.log(
      `📡 Using Art-Net broadcast address from environment: ${process.env.ARTNET_BROADCAST}`,
    );
  ...
  if (
    ...
    if (interfaces.length > 0) {
      ...
      console.log(
        `📡 ${reason} detected, using first available interface broadcast address: ${defaultAddress}`,
      );
      return defaultAddress;
    } else {
      console.log(
        `📡 ${reason} detected, no network interfaces found, using default broadcast address: 255.255.255.255`,
      );
    }
  ...
  try {
    ...
    const answer = await new Promise<string>((resolve) => {
      rl.question(
        `Select option [1-${interfaces.length}] (default: ${defaultIndex + 1}): `,
        (input) => {
          rl.close();
