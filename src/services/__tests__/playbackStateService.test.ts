...
describe("PlaybackStateService", () => {
  ...
  describe("startCue", () => {
    ...
    it("should schedule follow time when specified", async () => {
      ...
      mockPrisma.cueList.findUnique = jest
        .fn()
  ...
  describe("stopAllCueLists", () => {
    it("should stop all active cue lists", async () => {
      ...
      expect(
        playbackStateService.getPlaybackState("cue-list-1")!.isPlaying,
      ).toBe(true);
      expect(
        playbackStateService.getPlaybackState("cue-list-2")!.isPlaying,
      ).toBe(true);
  ...
  describe("jumpToCue", () => {
    ...
    it("should throw error for invalid cue index", async () => {
      await expect(
        playbackStateService.jumpToCue("test-cue-list", 5),
      ).rejects.toThrow("Invalid cue index");
      await expect(
        playbackStateService.jumpToCue("test-cue-list", -1),
      ).rejects.toThrow("Invalid cue index");
    });
    ...
    it("should throw error for non-existent cue list", async () => {
      ...
      await expect(
        playbackStateService.jumpToCue("non-existent", 0),
      ).rejects.toThrow();
    });
  });
  ...
  describe("fade progress emissions", () => {
    it("should emit fade progress updates during fade", async () => {
      ...
      const initialPublishCount = (mockPubSub.publish as jest.Mock).mock.calls
        .length;
      ...
      const recentCalls = (mockPubSub.publish as jest.Mock).mock.calls.slice(
        initialPublishCount,
      );
  ...
  describe("cleanup", () => {
    it("should clear all states and intervals", async () => {
      ...
      // Verify states exist
      expect(
        playbackStateService.getPlaybackState("test-cue-list-1"),
      ).not.toBeNull();
      expect(
        playbackStateService.getPlaybackState("test-cue-list-2"),
      ).not.toBeNull();
