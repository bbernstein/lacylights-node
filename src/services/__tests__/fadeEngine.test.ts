...
describe("FadeEngine", () => {
  ...
  describe("fade processing at 40Hz", () => {
    ...
    it("should verify 40Hz timing with 2048 channels across 4 universes", () => {
      ...
      const callsPerInterval = callTimes.filter(
        (t) => t === uniqueCallTimes[0],
      ).length;
  ...
  describe("fade cancellation", () => {
    it("should cancel specific fade", () => {
      const fadeId1 = fadeEngine.fadeChannels(
        [{ universe: 1, channel: 1, targetValue: 255 }],
        1,
      );
      const fadeId2 = fadeEngine.fadeChannels(
    ...
    it("should not process cancelled fades", () => {
      const fadeId = fadeEngine.fadeChannels(
        [{ universe: 1, channel: 1, targetValue: 255 }],
        1,
      );
  ...
  describe("edge cases", () => {
    it("should clamp channel values to 0-255 range", () => {
      ...
      expect(mockDmxService.setChannelValue).toHaveBeenCalledWith(
        1,
        1,
        expect.any(Number),
      );
    ...
    it("should handle multiple simultaneous fades on different channels", () => {
      ...
      fadeEngine.fadeChannels(
        [{ universe: 2, channel: 1, targetValue: 64 }],
        2,
        "fade3",
      );
